...
ColumnLayout {
    ...
    TideText { baseFont:view.textFont; Layout.fillWidth:true; visible:view.available; text:view.available ? view.n(view.usage.today.total_tokens)+' tokens today' : ''; sizeScale:(view.compact ? .9 : 1.15)*view.textScale; color:'#edf5fc'; wrapMode:Text.WordWrap }
    TideText {
        baseFont:view.textFont; Layout.fillWidth:true; visible:view.available && !view.compact
    ...
    RowLayout {
        visible:view.available && !view.compact;Layout.fillWidth:true;Layout.preferredHeight:62*view.textScale;Layout.maximumHeight:62*view.textScale;spacing:6
        ColumnLayout {
    ...
    TideText { baseFont:view.textFont;Layout.fillWidth:true;visible:view.available && !view.compact;text:'−60 min → latest scan · 5 min bins · tokens/min';sizeScale:.6*view.textScale;color:'#8298ac';wrapMode:Text.WordWrap }
    TideText { baseFont:view.textFont;Layout.fillWidth:true;visible:!view.compact;text:'Local profile, all accounts. Cached is included in input; reasoning in output. Not account-wide or billing usage.';sizeScale:.6*view.textScale;color:'#8298ac';wrapMode:Text.WordWrap }
}
